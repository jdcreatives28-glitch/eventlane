  ...
  useMapEvent('click', (e) => {
    ...
    if (domTarget && domTarget.closest) {
      ...
      if (
        ...
        domTarget.closest('.venue-card') || // popup card
...
/* ---------------- Main component (map-only) ---------------- */

export default function VenueMap() {
  const query = useQuery();
  const navigate = useNavigate();

  // 🔒 Full-screen map: hide global topbar + footernav + bottom padding
  useEffect(() => {
    const topbar = document.querySelector('.topbar, #topbar, .app-topbar');
    const footer = document.querySelector(
      '#footernav, .mobile-footer, .footer-nav, .app-footer, .bottom-nav'
    );
    ...
    document.body.style.paddingBottom = '0px'; // ✅ remove padding-bottom:72px;

    return () => {
      if (topbar) topbar.style.display = prevTopbarDisplay ?? '';
      if (footer) footer.style.display = prevFooterDisplay ?? '';
      document.body.style.paddingBottom = prevBodyPaddingBottom ?? '';
    };
  }, []);

  // Always scroll to top when opening map
  useEffect(() => {
    window.scrollTo({
      top: 0,
      left: 0,
  ...
  // This page is full-screen, no external topbar/footernav layout
  const mapHeight = 'calc(100vh - 180px)'; // space for back button + search/filters
  ...
  const { mapCenter, mapZoom, mapKey } = useMemo(() => {
    const sourceForCenter =
      filteredVenues && filteredVenues.length > 0 ? filteredVenues : venues;
...
const primaryBtn = {
          ...
          <Marker
            ...
            eventHandlers={{
              click: () => {
                ...
                // Leaflet will open the popup automatically
              },
            }}
          >
            <Popup
            ...
            >
              ...
              >
                ...
                <div style={{ padding: '10px 12px 12px' }}>
                  ...
                  >
                    {venue.location && chip(<FiMapPin size={12} />, venue.location)}
                    {venue.type && chip(<FiTag size={12} />, venue.type)}
                    {Number.isFinite(venue.capacity_max) &&
                      chip(<FiUsers size={12} />, `Up to ${venue.capacity_max}`)}
                    {Number.isFinite(distanceKm) &&
                      chip(<FiNavigation size={12} />, `${distanceKm.toFixed(1)} km`)}
                  </div>
                </div>
  ...
  /* ---------------- Render: back button + search + filters + map ---------------- */
  ...
  return (
    <div style={styles.container}>
      ...
      <div className="vm-shell">
        ...
        <div className="vm-controls">
          ...
          <div style={styles.pathRow} className="path-row">
            ...
            <div className="select-wrap">
              ...
              >
                <option value="">
                  {regionsLoading ? 'Loading regions…' : 'Region'}
                </option>
                {regions.map((r) => (
        ...
        {/* Map */}
        <div className="vm-map-wrap">{renderMap('-main')}</div>
      </div>
    </div>
  );
}

/* ---------------- Skeleton / select CSS injection ---------------- */
...
const styles = {
  ...
  backButton: {
    ...
    boxShadow: '0 10px 24px rgba(15,23,42,0.10), inset 0 1px 0 rgba(255,255,255,0.9)',
    color: textDark,
    fontSize: 13,
    fontWeight: 600,
    cursor: 'pointer',
...
function StyleOnce() {
  useEffect(() => {
    if (document.getElementById('venue-map-responsive')) return;
    const style = document.createElement('style');
    style.id = 'venue-map-responsive';
    style.textContent = `
      .filter-row{
        display:flex;
        flex-direction:column;
